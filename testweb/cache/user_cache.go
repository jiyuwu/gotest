package cache

import (
	"encoding/json"
	"fmt"

	"github.com/jiyuwu/gotest/testweb/vo"
	"gopkg.in/redis.v5"
)

const (
	userOnlinePrefix    = "acc:user:online:" // 用户在线状态
	userTokenPrefix     = "acc:user:token:"  // 用户token
	userOnlineCacheTime = 24 * 60 * 60
)

/*********************  查询用户是否在线  ************************/
func getUserOnlineKey(userKey string) (key string) {
	key = fmt.Sprintf("%s%s", userOnlinePrefix, userKey)

	return
}

func GetUserOnlineInfo(userKey string) (userOnline *vo.UserOnline, err error) {
	redisClient := GetClient()

	key := getUserOnlineKey(userKey)

	data, err := redisClient.Get(key).Bytes()
	if err != nil {
		if err == redis.Nil {
			fmt.Println("GetUserOnlineInfo", userKey, err)

			return
		}

		fmt.Println("GetUserOnlineInfo", userKey, err)

		return
	}

	userOnline = &vo.UserOnline{}
	err = json.Unmarshal(data, userOnline)
	if err != nil {
		fmt.Println("获取用户在线数据 json Unmarshal", userKey, err)

		return
	}

	fmt.Println("获取用户在线数据", userKey, "time", userOnline.LoginTime, userOnline.HeartbeatTime, "AccIp", userOnline.AccIp, userOnline.IsLogoff)

	return
}

// 设置用户在线数据
func SetUserOnlineInfo(userKey string, userOnline *vo.UserOnline) (err error) {

	redisClient := GetClient()
	key := getUserOnlineKey(userKey)

	valueByte, err := json.Marshal(userOnline)
	if err != nil {
		fmt.Println("设置用户在线数据 json Marshal", key, err)

		return
	}

	_, err = redisClient.Do("setEx", key, userOnlineCacheTime, string(valueByte)).Result()
	if err != nil {
		fmt.Println("设置用户在线数据 ", key, err)

		return
	}

	return
}

/*********************  查询用户token  ************************/
func getUserTokenKey(userKey string) (key string) {
	key = fmt.Sprintf("%s%s", userTokenPrefix, userKey)

	return
}

func GetUserTokenInfo(userKey string) (token string, err error) {
	redisClient := GetClient()

	key := getUserTokenKey(userKey)

	data, err := redisClient.Get(key).Bytes()
	if err != nil {
		if err == redis.Nil {
			fmt.Println("GetUserTokenInfo", userKey, err)

			return
		}

		fmt.Println("GetUserTokenInfo", userKey, err)

		return
	}
	token = string(data)
	return
}

// 设置用户token
func SetUserTokenInfo(userKey string, token string) (err error) {

	redisClient := GetClient()
	key := getUserTokenKey(userKey)
	_, err = redisClient.Do("setEx", key, userOnlineCacheTime, token).Result()
	if err != nil {
		fmt.Println("设置用户token ", key, err)

		return
	}

	return
}
